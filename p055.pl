...
# The total count of Lychrel numbers.
$count = 0;

# Iterate through the range.
for $n (1 .. $max) {
	# Increment the counter if this number is Lychrel.
	++$count if (is_lychrel($n));
}

# Print the result.
print "Found: $count\n";

# This is used to determine whether a number is a Lychrel number.
sub is_lychrel {
	# Get the parameter.
	my $n = shift;

	# The number if iterations to do.
	my $i = 50;

	# Iterate until we don't need to iterate any more.
	while ($i--) {
		# Add the reverse.
		$n += reverse($n);

		# Break out if it is a palindrome.
		last if ($n eq reverse($n));
	}

	# Return whether we found a palindrome.
	return $i == -1;
}
